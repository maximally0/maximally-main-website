...
const ShipathonReport = () => {
  const eventStats = [
    { icon: Users, label: 'Global Participants', value: '610', gradient: 'from-blue-600/40 to-gray-700/30', border: 'border-blue-500/50', textColor: 'text-blue-400' },
    { icon: Zap, label: 'AI Project Submissions', value: '100+', gradient: 'from-orange-600 to-orange-500', border: 'border-orange-500/50', textColor: 'text-orange-400' },
  ...
  const highlights = [
    { title: 'Organic Growth Success', description: "Event grew organically with minimal promotion, showing Maximally's brand strength", icon: Rocket, gradient: 'from-orange-600 to-orange-500', border: 'border-orange-500/50' },
    { title: 'AI Experimentation Focus', description: 'Encouraged creativity, beginner-friendly participation, and team collaboration', icon: Zap, gradient: 'from-blue-600/40 to-gray-700/30', border: 'border-blue-500/50' },
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-black text-white relative overflow-hidden">
        ...
        <div className="absolute bottom-40 right-20 w-80 h-80 bg-orange-500/12 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '1s' }} />

        <div className="container mx-auto px-4 pt-24 pb-16 relative z-10">
          {/* Header */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
            <Link to="/events" className="inline-flex items-center gap-2 font-space text-orange-400 hover:text-orange-400 transition-colors mb-6 group">
              <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
              Back to Events
            </Link>

            <div className="text-center">
              <motion.h1 className="font-space font-bold text-3xl md:text-5xl mb-6 bg-gradient-to-r from-blue-400 via-orange-400 to-orange-400 bg-clip-text text-transparent leading-tight">
                Maximally AI Shipathon
              </motion.h1>
              <div className="flex items-center justify-center gap-6 mb-4 flex-wrap">
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-orange-400" />
                  <span className="font-space text-gray-300">Aug 30 - Sept 1, 2025</span>
                </div>
                <div className="flex items-center gap-2">
                  <Clock className="h-4 w-4 text-orange-400" />
                  <span className="font-space text-gray-300">48 Hours</span>
                </div>
              </div>
              <p className="font-space text-lg text-gray-300 max-w-3xl mx-auto leading-relaxed">
              ...
              {eventStats.map((stat, index) => (
                <motion.div key={index} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.1 }} whileHover={{ scale: 1.05 }}>
                  <div className={`bg-gradient-to-br ${stat.gradient} border ${stat.border} p-6 text-center transition-all hover:scale-[1.02]`}>
                    ...
                    <div className="font-space font-bold text-2xl text-white mb-2">{stat.value}</div>
                    <div className="font-space text-sm text-gray-400">{stat.label}</div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.section className="mb-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
            <div className="text-center mb-12">
              <h2 className="font-space font-bold text-2xl md:text-3xl mb-4 bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Project Categories</h2>
              <div className="w-32 h-1 bg-gradient-to-r from-orange-600 to-orange-500 mx-auto" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {projectTypes.map((type, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + index * 0.1 }} whileHover={{ scale: 1.05 }}>
                  <div className="bg-gradient-to-br from-gray-900/40 to-gray-900/20 border border-orange-500/30 p-6 text-center hover:border-orange-500/50 transition-all">
                    <div className="text-4xl mb-3">{type.icon}</div>
                    <div className="font-space font-bold text-xs text-orange-400 mb-2">{type.label}</div>
                    <div className="font-space text-xs text-gray-400">{type.description}</div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.section className="mb-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
            <div className="text-center mb-12">
              <h2 className="font-space font-bold text-2xl md:text-3xl mb-4 bg-gradient-to-r from-blue-400 to-gray-500 bg-clip-text text-transparent">Shipathon Highlights</h2>
              <div className="w-32 h-1 bg-gradient-to-r from-orange-600 to-orange-500 mx-auto" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {highlights.map((highlight, index) => (
                <motion.div key={index} initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + index * 0.1 }} whileHover={{ scale: 1.02 }}>
                  <div className={`bg-gradient-to-br ${highlight.gradient} border ${highlight.border} p-8 h-full hover:border-orange-500/50 transition-all`}>
                    <highlight.icon className="h-8 w-8 text-orange-400 mb-4" />
                    <h3 className="font-space font-bold text-sm text-white mb-4">{highlight.title}</h3>
                    <p className="font-space text-gray-300 text-sm leading-relaxed">{highlight.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.section className="mb-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
            <div className="text-center mb-12">
              <h2 className="font-space font-bold text-2xl md:text-3xl mb-4 bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">Global Judges & Mentors</h2>
              <div className="w-32 h-1 bg-gradient-to-r from-amber-600 to-orange-600 mx-auto" />
              <p className="font-space text-gray-400 mt-4 max-w-2xl mx-auto">Engineers, founders, and product leaders from Meta, Atlassian, Warner Bros., McKinsey, Oracle, and YC-backed startups</p>
            </div>
              ...
              {judges.map((judge, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + index * 0.05 }} whileHover={{ scale: 1.02 }}>
                  <div className="bg-gradient-to-br from-amber-900/30 to-orange-900/20 border border-amber-500/40 p-6 hover:border-orange-400/50 transition-all h-full">
                    <h3 className="font-space font-bold text-xs text-white mb-3">{judge.name}</h3>
                    <p className="font-space text-gray-400 text-xs leading-relaxed">{judge.role}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.section className="mb-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
            <div className="text-center mb-12">
              <h2 className="font-space font-bold text-2xl md:text-3xl mb-4 bg-gradient-to-r from-orange-400 to-blue-400 bg-clip-text text-transparent">Event Links</h2>
              <div className="w-32 h-1 bg-gradient-to-r from-orange-600 to-orange-500 mx-auto" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {socialLinks.map((link, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + index * 0.1 }} whileHover={{ scale: 1.02 }}>
                  <div className="bg-gradient-to-br from-gray-900/30 to-blue-900/20 border border-gray-700 p-6 hover:border-blue-400/50 transition-all">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-space font-bold text-xs text-white mb-2">{link.title}</h3>
                        <p className="font-space text-xs text-gray-400">{link.platform}</p>
                      </div>
                      <a href={link.url} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-r from-orange-600 to-orange-500 hover:from-gray-700 hover:to-blue-500 text-white font-space font-bold text-xs px-4 py-2 border border-gray-700 transition-all flex items-center gap-2">
          ...
          {/* Impact Statement */}
          <motion.section className="text-center bg-gradient-to-br from-gray-900/40 to-gray-900/20 border border-orange-500/30 p-12 md:p-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0 }}>
            <h2 className="font-space font-bold text-2xl md:text-3xl mb-8 bg-gradient-to-r from-orange-400 via-orange-400 to-blue-400 bg-clip-text text-transparent">Legacy & Next Steps</h2>
            <p className="font-space text-lg text-gray-300 mb-6 max-w-4xl mx-auto">
            ...
            <p className="font-space text-base text-gray-400 max-w-3xl mx-auto mb-8">
            ...
            <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600/30 to-orange-600/20 border border-blue-500/40">
              <Code2 className="h-5 w-5 text-blue-400" />
              <span className="font-space text-sm text-gray-300">Next: Grand Indian Hackathon Season</span>
            </div>
          </motion.section>
        </div>
