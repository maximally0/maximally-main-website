...
import type {
  ...
  UnreadCountResponse,
  MarkReadResponse
} from '@/types/organizerMessages';

const API_BASE = '/api/organizer';

/**
 * Fetch messages for the authenticated organizer
 */
export async function getOrganizerMessages(filters?: MessageFilters): Promise<MessagesResponse> {
  try {
 ...
 */
export async function markOrganizerMessageRead(id: number): Promise<MarkReadResponse> {
  try {
    const headers = await getAuthHeaders();
 ...
 */
export async function getOrganizerUnreadCount(): Promise<UnreadCountResponse> {
  try {
    const headers = await getAuthHeaders();
