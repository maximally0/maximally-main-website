...
import { useJudgeMessages } from '@/hooks/useJudgeMessages';
import { useJudgeUnreadCount } from '@/hooks/useJudgeUnreadCount';
import { markMessageRead } from '@/api/judgeMessages';
import type { MessageWithRead, Priority } from '@/types/judgeMessages';
...
const JudgeInbox = () => {
  ...
  const { refetch: refetchUnreadCount } = useJudgeUnreadCount();
  ...
  const { messages, loading, error, refetch, setFilters: updateFilters } = useJudgeMessages(filters);
  
  // Auth guard - only judges can access
  useEffect(() => {
  ...
  // Show loading while auth is loading OR profile is not loaded yet
  ...
  return (
    <>
      <SEO
        title="Judge Inbox | Maximally"
        description="View messages from the Maximally admin team"
      />
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="font-jetbrains text-gray-400 text-sm mb-3 line-clamp-2">
                            {message.content}
                          </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              ...
              {selectedMessage.recipient.is_read && selectedMessage.recipient.read_at && (
                <p className="font-jetbrains text-xs text-gray-500 text-center">
                  Read on {formatDate(selectedMessage.recipient.read_at)}
                </p>
              )}
            </div>
          </div>
