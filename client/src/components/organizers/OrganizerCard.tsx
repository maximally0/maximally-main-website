...
const tierConfig: Record<OrganizerTier, { label: string; color: string; bgColor: string }> = {
  ...
  senior: { label: 'SENIOR', color: 'text-orange-400', bgColor: 'bg-orange-500/10 border-orange-500' },
  ...
  return (
    <div className={`pixel-card bg-gray-900 border-2 ${tier.bgColor} p-4 hover:scale-105 transition-transform duration-200`}>
      ...
      <div className="flex justify-between items-start mb-3">
        <span className={`font-space font-bold text-xs ${tier.color}`}>{tier.label}</span>
        <div className="flex gap-2">
          ...
          {organizer.twitter && (
            <a href={organizer.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-gray-300">
      ...
      <div className="text-center mb-3">
        <h3 className="font-space font-bold text-sm text-green-400 mb-1 truncate">{organizer.fullName}</h3>
        <p className="font-space text-xs text-gray-400 line-clamp-2">{organizer.headline}</p>
      </div>
      ...
      {organizer.organizationName && (
        <div className="flex items-center justify-center gap-2 mb-3 text-xs text-gray-500">
          ...
          <span className="font-space truncate">{organizer.organizationName}</span>
        </div>
      )}
      ...
      {organizer.location && (
        <div className="flex items-center justify-center gap-2 mb-3 text-xs text-gray-500">
          ...
          <span className="font-space">{organizer.location}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-black/50 rounded p-2 text-center">
          ...
          <p className="font-space font-bold text-xs text-white">{organizer.totalHackathonsHosted}</p>
          <p className="font-space text-xs text-gray-500">Events</p>
        </div>
        <div className="bg-black/50 rounded p-2 text-center">
          <Users className="h-4 w-4 text-gray-300 mx-auto mb-1" />
          <p className="font-space font-bold text-xs text-white">{organizer.totalParticipantsReached}</p>
          <p className="font-space text-xs text-gray-500">Reached</p>
        </div>
      </div>
      ...
      <Link
        ...
        className="block w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white text-center py-3 font-space font-bold text-xs border border-green-500/50 transition-all hover:scale-[1.02]"
      >
        VIEW PROFILE
      </Link>
    </div>
