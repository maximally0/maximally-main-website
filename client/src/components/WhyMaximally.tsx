  ...
  return (
    <div className={`flex flex-col items-center transition-all duration-300 ${isActive ? "opacity-100 scale-110" : "opacity-40"}`}>
      <div className={`w-12 h-12 ${isActive ? colors.bg : 'bg-gray-800/50'} border ${isActive ? colors.border : 'border-gray-700'} flex items-center justify-center transition-all duration-300`}>
        <span className={`font-space font-bold text-sm ${isActive ? colors.text : 'text-gray-500'}`}>{number}</span>
      </div>
      <span className={`font-space text-sm mt-2 ${isActive ? 'text-white' : 'text-gray-500'}`}>{label}</span>
    </div>
  );
};
...
const WhyMaximally = () => {
  ...
  return (
    <section id="why" className="py-16 sm:py-24 bg-black relative overflow-hidden">
      ...
      <div className="absolute top-20 left-[10%] w-60 h-60 bg-orange-500/10 rounded-full blur-[80px]" />
      <div className="absolute bottom-20 right-[10%] w-48 h-48 bg-orange-500/5 rounded-full blur-[60px]" />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12">
            {features.map((feature, index) => (
              <div 
                key={index} 
                className={`group bg-gradient-to-br ${feature.gradient} border ${feature.border} p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(249,115,22,0.08)]`}
              >
                <div className={`${feature.iconBg} border ${feature.border} w-14 h-14 flex items-center justify-center mb-4 mx-auto`}>
                  <feature.icon className={`h-7 w-7 ${feature.iconColor}`} />
                </div>
                <h3 className="font-space font-bold text-xs sm:text-sm text-white mb-3 text-center group-hover:text-orange-400 transition-colors">
                  {feature.title}
                </h3>
                <p className="font-space text-sm text-gray-400 text-center leading-relaxed">
                  {feature.description}
                </p>
              </div>
            ))}
          ...
          <div className="bg-gradient-to-br from-gray-900/60 to-gray-900/30 border border-gray-800 p-6 sm:p-8">
            <h3 className="font-space font-bold text-sm sm:text-base text-white mb-8 text-center">
              Your Growth Journey
            </h3>
            
            <div className="flex justify-between items-center">
              <EvolutionStep number="1" label="Noob" isActive={activeStep >= 1} color={stepColors[0]} />
              <div className={`h-1 flex-1 mx-2 transition-all duration-300 ${activeStep >= 2 ? 'bg-gradient-to-r from-orange-500/50 to-orange-500/50' : 'bg-gray-700'}`}></div>
              <EvolutionStep number="2" label="Starter" isActive={activeStep >= 2} color={stepColors[1]} />
              <div className={`h-1 flex-1 mx-2 transition-all duration-300 ${activeStep >= 3 ? 'bg-gradient-to-r from-orange-500/30 to-gray-700/50' : 'bg-gray-700'}`}></div>
              <EvolutionStep number="3" label="Pro" isActive={activeStep >= 3} color={stepColors[2]} />
              <div className={`h-1 flex-1 mx-2 transition-all duration-300 ${activeStep >= 4 ? 'bg-gradient-to-r from-gray-700/50 to-green-500/50' : 'bg-gray-700'}`}></div>
