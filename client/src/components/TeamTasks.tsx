...
export default function TeamTasks({ teamId }: TeamTasksProps) {
  ...
  if (loading) {
    return <div className="text-center py-8 text-gray-400 font-space">Loading tasks...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        >
          ...
          <span className="font-space font-bold text-xs">ADD TASK</span>
        </button>
      </div>
      ...
      {showAddTask && (
        <div className="bg-gradient-to-br from-gray-900/40 to-gray-900/20 border border-gray-700 p-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-space font-bold text-sm text-gray-300 flex items-center gap-2">
        ...
        {Object.entries(groupedTasks).map(([status, statusTasks]) => (
          <div key={status} className="space-y-3">
            <div className={`border ${statusColors[status as keyof typeof statusColors]} px-3 py-2 text-center`}>
              <span className={`font-space font-bold text-[10px] ${statusTextColors[status as keyof typeof statusTextColors]} whitespace-nowrap`}>
            ...
            <div className="space-y-2">
              {statusTasks.map((task) => (
                <div
                  key={task.id}
                  className="bg-gradient-to-br from-gray-900/60 to-gray-900/30 border border-gray-700 hover:border-gray-700 p-4 transition-all duration-300 cursor-pointer"
                  onClick={() => {
                ...
                >
                  <div className="flex items-start gap-2 mb-2">
                    ...
                    <h5 className="font-space text-sm text-white flex-1">
                      {task.task_title}
                    </h5>
                  </div>
                  ...
                  <div className={`inline-block px-2 py-1 border font-space font-bold text-[10px] ${getPriorityColor(task.priority)}`}>
      ...
      {tasks.length === 0 && (
        <div className="text-center py-12">
          <div className="bg-gray-800/50 border border-gray-700 text-gray-400 px-6 py-4 inline-block">
            <span className="font-space font-bold text-sm">NO TASKS YET</span>
          </div>
