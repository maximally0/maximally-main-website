...
import { Menu, X, Terminal, Mail, User, LogOut, Trophy, ChevronDown } from "lucide-react";
...
const Navbar = () => {
  ...
  const navKeys = Object.keys(dropdownMenus) as (keyof typeof dropdownMenus)[];
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center lg:hidden">
            <button className="p-2 bg-gray-900 border border-gray-700 hover:border-orange-500 text-white transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu" data-testid="button-menu-toggle">
                ...
                <div className="border-t border-gray-800 pt-4 mt-2">
                  {loading || (user && !profile) ? (
                    ...
                    <div className="space-y-2">
                      {isOrganizer && (
                        <>
                          <Link to="/organizer/dashboard" onClick={() => setIsMenuOpen(false)} className="block font-space text-center py-3.5 px-6 text-sm font-medium bg-gray-900/30 text-orange-400 border border-orange-500/50 hover:bg-gray-900/50 transition-all duration-200">Organizer Dashboard</Link>
                          <Link to="/organizer-inbox" onClick={() => setIsMenuOpen(false)} className="block font-space text-center py-3.5 px-6 text-sm font-medium bg-gray-900/30 text-orange-400 border border-orange-500/50 hover:bg-gray-900/50 transition-all duration-200 relative">
                            Organizer Inbox
                            {organizerUnreadCount > 0 && <span className="ml-2 bg-green-500 text-white text-xs font-space px-2 py-0.5 rounded-full">{organizerUnreadCount > 99 ? "99+" : organizerUnreadCount}</span>}
                          </Link>
                        </>
                      )}
                      <Link to={profileUrl} onClick={() => setIsMenuOpen(false)} className="block font-space text-center py-3.5 px-6 text-sm font-medium bg-gray-900 text-white border border-gray-700 hover:border-orange-500 transition-all duration-200">My Profile</Link>
                      {!isOrganizer && (
                        <Link to="/my-hackathons" onClick={() => setIsMenuOpen(false)} className="block font-space text-center py-3.5 px-6 text-sm font-medium bg-gray-900 text-white border border-gray-700 hover:border-orange-500 transition-all duration-200">My Hackathons</Link>
                      )}
                      <button onClick={() => { handleSignOut(); setIsMenuOpen(false); }} className="w-full font-space text-center py-3.5 px-6 text-sm font-medium bg-gray-900 text-red-400 border border-gray-700 hover:bg-red-900/20 hover:border-red-500 transition-all duration-200">Sign Out</button>
                    </div>
                  ) : (
                    <Link to="/login" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-center py-4" data-testid="button-login-mobile" aria-label="Sign in">
