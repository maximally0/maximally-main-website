...
export function registerProfileRoutes(app: Express) {
  ...
  app.get('/api/judges', async (req: Request, res: Response) => {
    try {
      ...
      if (error) {
        console.error('[ProfileRoutes] Error fetching judges:', error)
      ...
      const formattedJudges = judges?.map(judge => ({
        ...judge,
        evaluation_status: judge.evaluation_status || 0,
        judge_tier: judge.judge_tier || 'junior',
        total_evaluations_completed: judge.total_evaluations_completed || 0,
        hackathons_judged: judge.hackathons_judged || []
      })) || []

      return res.json(formattedJudges)
    } catch (error) {
      console.error('[ProfileRoutes] Error in GET /api/judges:', error)
      return res.status(500).json({ error: 'Internal server error' })
    }
  })

  // GET /api/organizers - Get all organizers
  app.get('/api/organizers', async (req: Request, res: Response) => {
    try {
      const supabaseAdmin = req.app.locals.supabaseAdmin;
      if (!supabaseAdmin) {
        return res.status(500).json({ error: 'Server not configured' })
      ...
      if (error) {
        console.error('[ProfileRoutes] Error fetching organizers:', error)
      ...
      const formattedOrganizers = organizers?.map(organizer => ({
        ...organizer,
        organizer_status: organizer.organizer_status || 'inactive',
        events_organized: organizer.events_organized || 0,
        total_participants_managed: organizer.total_participants_managed || 0
      })) || []

      return res.json(formattedOrganizers)
    } catch (error) {
      console.error('[ProfileRoutes] Error in GET /api/organizers:', error)
      return res.status(500).json({ error: 'Internal server error' })
    }
  })
}