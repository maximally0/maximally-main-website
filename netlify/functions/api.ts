...
app.get("/api/hackathons/:hackathonId/is-organizer", async (req, res) => {
  try {
    const authHeader = req.headers['authorization'];
    if (!authHeader?.startsWith('Bearer ')) return res.json({ success: true, isOrganizer: false });
    const token = authHeader.slice('Bearer '.length);
    const userId = await bearerUserId(supabaseAdmin, token);
    if (!userId) return res.json({ success: true, isOrganizer: false });
    const { hackathonId } = req.params;
    const { data: hackathon } = await (supabaseAdmin as any).from('organizer_hackathons').select('organizer_id').eq('id', hackathonId).single();
    if (hackathon?.organizer_id === userId) return res.json({ success: true, isOrganizer: true });
    const { data: coOrg } = await (supabaseAdmin as any).from('hackathon_organizers').select('role, status').eq('hackathon_id', hackathonId).eq('user_id', userId).eq('status', 'accepted').single();
    return res.json({ success: true, isOrganizer: !!coOrg });
  } catch (e: any) { return res.json({ success: true, isOrganizer: false }); }
});
...
app.get("/api/hackathons/:hackathonId/winners", async (req, res) => {
  try {
    ...
    const { hackathonId } = req.params;
    const hackathonIdNum = parseInt(hackathonId);
    if (isNaN(hackathonIdNum)) return res.status(400).json({ success: false, message: 'Invalid hackathon ID' });
    const { data, error } = await (supabaseAdmin as any).from('hackathon_winners').select(`*, submission:hackathon_submissions(id, project_name, description, tagline, github_repo, demo_url, video_url, cover_image, project_logo, user_id, team_id)`).eq('hackathon_id', hackathonIdNum).order('position', { ascending: true });
    if (error) throw error;
    const enrichedData = await Promise.all((data || []).map(async (winner: any) => {
      let userName = 'Anonymous';
      let teamName = null;
      ...
      return { ...winner, submission: winner.submission ? { ...winner.submission, user_name: userName, team: teamName ? { team_name: teamName } : null } : null, team_name: teamName };
    }));
    return res.json({ success: true, data: enrichedData });
  } catch (e: any) { return res.status(500).json({ success: false, message: e.message }); }
});
...
app.get("/api/hackathons/:hackathonId/announcements", async (req, res) => {
  try {
    if (!supabaseAdmin) return res.status(500).json({ success: false, message: "Server not configured" });
    const { hackathonId } = req.params;
    const { data, error } = await (supabaseAdmin as any).from('hackathon_announcements').select('*').eq('hackathon_id', hackathonId).eq('is_published', true).eq('target_audience', 'public').order('published_at', { ascending: false });
    if (error) throw error;
    return res.json({ success: true, data: data || [] });
  } catch (e: any) { return res.status(500).json({ success: false, message: e.message }); }
});
