...
const colorClasses = {
  purple: {
    ...
    badge: "bg-orange-500/10 border-gray-800 text-orange-400"
  },
  pink: {
    bg: "bg-orange-500/10",
    border: "border-orange-500/30",
    text: "text-orange-400",
    badge: "bg-orange-500/5 border-gray-800 text-orange-400"
  },
  cyan: {
    bg: "bg-gray-800",
    ...
    <div className={`bg-gradient-to-br from-gray-900/60 to-gray-900/30 border ${colors.border} h-full flex flex-col p-4 sm:p-6 transition-all duration-300 hover:border-opacity-70`}>
      <div className="flex items-center mb-4">
        <div className={`w-10 sm:w-12 h-10 sm:h-12 ${colors.bg} border ${colors.border} flex items-center justify-center mr-4`}>
          <span className={`font-space font-bold ${colors.text} text-sm sm:text-base`}>{testimonial.avatar}</span>
        </div>
        <div>
          <p className="font-space font-bold text-sm text-white">{testimonial.name}</p>
          <p className="font-space text-xs text-gray-400">{testimonial.handle}</p>
        </div>
      </div>
      ...
      <div className="relative flex-grow mb-4">
        ...
        <p className="font-space text-sm sm:text-base text-gray-300 pl-4">{testimonial.quote}</p>
      </div>
      
      <div className={`${colors.badge} border px-3 py-1 font-space text-xs inline-block self-start`}>
        ...
        <div className="relative">
          ...
          <div className="md:hidden">
            ...
            <div className="flex justify-center mt-6 gap-4">
              <button 
                onClick={prevSlide}
                className="p-2 bg-orange-500/10 border border-orange-500/30 text-orange-400 hover:bg-orange-500/20 transition-colors"
